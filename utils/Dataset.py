...
#         super().__init__()
#         n_labels = config["n_labels"]  # 12
#         n_maps = config["n_feature_maps"]  # 45
#         self.conv0 = nn.Conv2d(1, n_maps, (3, 3), padding=(1, 1), bias=False)
#         #  input_channel=1, output_channel=1, kernel_size=(3,3), stride=1,padding=(1,1)
#         if "res_pool" in config:
#             self.pool = nn.AvgPool2d(config["res_pool"])  # [4,3] pooling
#
#         self.n_layers = n_layers = config["n_layers"]  # 6
#         # self.n_layers = n_layers = 4
#         dilation = config["use_dilation"]  # False
#         if dilation:
...
#         # x = torch.no_grad(torch.zeros(1, 1, height, width), volatile=True)
...
class WavDataset(data.Dataset):
    def __init__(self, file):
        super().__init__()
        self.dataset = h5py.File(file, 'r', swmr=True)
        self.data = self.dataset["data"]  # np.asarray(self.train_data)
